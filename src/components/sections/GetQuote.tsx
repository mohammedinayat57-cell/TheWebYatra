...
export default function GetQuote() {
  ...
  const handleGetQuote = async () => {
    ...
    try {
      ...
      row("Pages", `${pagesOption?.label ?? "—"}`,
        pagesExtra > 0 ? `+${fmt(pagesExtra)}` : "Included",
        pagesExtra === 0 ? [34, 197, 94] : undefined);
      row("Timeline", `${timelineOpt?.label ?? "—"}`,
        timelineMod > 0 ? `+${fmt(timelineMod)} rush fee`
          : timelineMod < 0 ? `-${fmt(Math.abs(timelineMod))} discount`
          : "No change",
      ...
      if (included.length) {
        ...
        doc.setFont("helvetica", "bold");
        doc.setTextColor(28, 26, 24);
        doc.text("Always Included (Free)", 14, y);
        y += 7;
        doc.setFont("helvetica", "normal");
        doc.setFontSize(9);
        doc.setTextColor(34, 197, 94);
        included.forEach(l => { doc.text(`✓  ${l}`, 18, y); y += 6; });
        y += 3;
      }
      ...
      if (addons.length) {
        ...
        addons.forEach(f => { doc.text(`+  ${f.label}  (+${fmt(f.cost)})`, 18, y); y += 6; });
        y += 3;
      }
    ...
    // ── Open WhatsApp ─────────────────────────────────────────────────────────
    const fl = selectedFeatures;
    const msg = [
      `*Quote Request — TheWebYatra* 🚀`, ``,
      `*Project:* ${projectType?.emoji} ${projectType?.label ?? "—"} (${fmt(basePrice)})`,
      ...
      `*Features:*`,
      ...fl.map(f => `  • ${f.label}${f.cost > 0 ? ` (+${fmt(f.cost)})` : ""}`),
      ``,
